...
(function(window,document,$){
	...
	Editor.DefaultSettings = {
		 ...
		 grainUrlObj: 'document.getElementById("pen1")',  // 笔刷纹理图片对象 
		 snap: [],                             // 保存会画之前的快照，用于撤销
	...
	Editor.prototype.init = function(){
        ...
        this.canvas.height = height * 0.5082;  // 设计图中canvas高度/页面高度  678 /1334
        ...
        if( this.context ){
    		...
    		this.l = 5;            // 控制线宽，功能同brushSize，只是做一个中间存储变量
        	this.initEvent();
        } 
	}
	...
	Editor.prototype.setSize = function( size ){
		if(this.options.isEraser ) {
			...
			this.l = this.options.brushSize = Math.ceil(size/2);
		}
	}

	// 设置笔刷纹理
	Editor.prototype.setGrain = function( imgObj ){
		this.options.grainUrlObj = imgObj;
	}
	...
	Editor.prototype.initEvent = function(){
	        ...
	        self.canvas.addEventListener("touchmove",self.touchMove = function(event){
			    ...
			    if(self.options.isEraser){
					...
					for (var i = 0; i < len; i++) {
						...
						self.context.drawImage(self.options.grainUrlObj,x,y,self.l,self.l);
						self.l = self.l - 0.2;

						if( self.l < (self.options.brushSize / 2)){
							self.l = Math.ceil(self.options.brushSize / 2);
						} 
					}
			    }
		    },false);  
        ...
        this.canvas.addEventListener("touchend",function(event){  
			//event.preventDefault();
			self.l = self.options.brushSize;
			if(!self.options.isEraser && self.pointArr.length >60){
				for(var j = 0; j <40 ;j++){
				   self.pointArr[j].x = self.pointArr[j].x-self.l/4;
				   self.pointArr[j].y = self.pointArr[j].y - self.l/4;          
				   self.context.drawImage(self.options.grainUrlObj,self.pointArr[j].x+2,self.pointArr[j].y+2,self.l,self.l);
				   self.l = self.l - 0.25;
				   if( self.l < (self.options.brushSize / 2)){
				   	self.l = Math.ceil(self.options.brushSize / 2)
				   } 
			...
			if (!self.options.isEraser && self.pointArr.length==1) {
				self.context.drawImage(self.options.grainUrlObj,self.pointArr[0].x1 - self.l/2,self.pointArr[0].y1 - self.l/2,self.l,self.l);		
			}
			self.pointArr = [];
